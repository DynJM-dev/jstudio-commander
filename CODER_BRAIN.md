...
- `12969cb` feat(hook): auto-learn claude_session_id on deterministic-strategy match
- `7968b0a` feat(status): 90s stale-activity force-idle guard (working → idle only)
- `3586e8c` feat(ui): SessionCard ⚠ stale? badge — status=working + no activity >60s
- `f81f87f` test(phase-u): auto-learn + force-idle + stale-badge (24 tests across 3 files)

**Problem (team-lead post-Phase-T live test):** Phase T's spawn-bind closed the bootstrap gap for Commander-spawned sessions (`[spawn-bind]` bound in 3.8s, `hookMatchStats.skipped = 0`) but two residual failure modes still produced stuck UI state. Mode 1: PM-spawned teammates (via PM's `TaskCreate`/Agent tool) never hit `/api/sessions` so Patch 0 never fired — `coder@ovagas-r2` had NULL `claude_session_id` and stale subdir transcript_paths while showing `status=working` for 2.7 hours against an idle pane. Mode 2: after the 60s `last_hook_at` yield expired, the poller's pane regex could still latch onto a lingering past-tense verb (`✻ Composing` / `Composing response`) and stay pinned to `working` for 1100+ seconds on a quiescent session.

**Patch 1 — auto-learn claude_session_id (file: `server/src/routes/hook-event.routes.ts`).** Exported `DETERMINISTIC_STRATEGIES` set containing exactly four values: `claudeSessionId`, `transcriptUUID`, `sessionId-as-row`, `cwd-exclusive`. The two rotation heuristics (`pm-cwd-rotation`, `coder-team-rotation`) are deliberately excluded because their "exactly one plausible candidate in this cwd/team" match can retroactively be revealed as wrong once a sibling session spawns seconds later — persisting the wrong UUID into `claude_session_id` would permanently mis-route that sibling's hooks. Exported helper `maybeAutoLearnClaudeSessionId(matchId, strategy, transcriptPath): boolean` runs `UPDATE sessions SET claude_session_id = ? WHERE id = ? AND claude_session_id IS NULL` — the `IS NULL` guard makes concurrent hooks racing on the same row idempotent (whichever write lands first wins; the second is a 0-row UPDATE). Call site lives in `processHook` right after `hookMatchStats[match.strategy] += 1` and before the `bumpLastActivity` call. Emits `session:updated` on successful backfill so the client re-renders immediately. Closes Mode 1 — the next hook for that teammate hits the O(1) primary `claudeSessionId` strategy.

**Patch 2 — 90s stale-activity force-idle (file: `server/src/services/status-poller.service.ts`).** Exported `STALE_ACTIVITY_MS = 90_000` sits above the 60s `HOOK_YIELD_MS` — any real active turn refreshes `last_activity_at` on nearly every tool-use hook, so anything crossing 90s of silence is overwhelmingly routing/classifier drift rather than a legitimate long turn. SELECT now reads `last_activity_at` alongside `last_hook_at`. Ordering inside the per-row loop (documented inline): (1) Phase T `last_hook_at` yield — if `msSinceHook < HOOK_YIELD_MS`, `continue` — recent hook wins so a real active turn can't be prematurely force-idled; (2) NEW stale-activity force-idle — if `session.status === 'working' && msSinceActivity > STALE_ACTIVITY_MS`, `UPDATE SET status='idle' WHERE id=? AND status='working'`, emit `session:status` with evidence `stale-activity-force-idle (Ns)`, record in flip-history ring buffer, set `lastKnownStatus.set(id, 'idle')`, `workingSince.delete(id)`, `idleSince.delete(id)`, then `continue`; (3) pane classifier below. The UPDATE's `AND status = 'working'` WHERE clause prevents racing a concurrent Stop hook (which flips to idle) into a redundant write. Scope is deliberately narrow: ONLY flips `working` → `idle`. Idle / waiting / stopped rows pass through to the classifier untouched so a permission prompt still surfaces urgently. Also exported `HOOK_YIELD_MS` and `STALE_ACTIVITY_MS` (previously module-private) so tests pin by import instead of duplicate constant values.

**Patch 3 — SessionCard `⚠ stale?` chip (file: `client/src/components/sessions/SessionCard.tsx`).** Exported pure predicate `shouldShowStaleBadge(rawStatus, secondsAgo): boolean` and exported `STALE_BADGE_THRESHOLD_S = 60`. Chip renders next to the StatusBadge when `session.status === 'working'` AND `useHeartbeat(...).secondsAgo > 60`. The 60s threshold deliberately sits above the 30s band that drives `applyStaleOverride` — the override already visually coerces working/waiting → idle after 30s, so the chip only appears when the discrepancy is clearly beyond that existing grace window. Pure display: the server remains authoritative. Small orange `text-[10px]` Montserrat chip with tooltip `"Status shows working but no activity in X seconds — may be a routing gap."`. The chip is visibility-only — if Patches 1/2 work correctly the chip should rarely render in the wild; when it does render it's evidence of a regression that needs investigation.

**Patch 4 — tests (3 files, 24 tests).**

- `server/src/services/__tests__/phase-u-auto-learn.test.ts` (10 tests): DETERMINISTIC_STRATEGIES set size pinned to exactly 4 with explicit has/not-has checks; one positive case per deterministic strategy; pm-cwd-rotation MUST NOT fire; coder-team-rotation MUST NOT fire; already-populated row no-op (preserves original value); non-UUID transcript filename no-op; unknown session id no-op (UPDATE affects 0 rows). Uses `COMMANDER_DATA_DIR` tmpdir + real sqlite.
- `server/src/services/__tests__/phase-u-force-idle.test.ts` (9 tests): constants pinned (60_000 / 90_000); pure-decision `decide(status, msSinceActivity, msSinceHook)` mirrors production ordering and walks every boundary (91s force-idle, 89s and 90s-exact boundary no-change via strict `>`, idle/waiting/stopped scope guards, hook-yield-wins-over-force-idle @ 1s, 60s-exact boundary flows into force-idle); one E2E-light integration test seeds a working + 120s-stale row, calls `statusPollerService.start()` (which runs `poll()` synchronously before the interval arms), immediately calls `.stop()`, and asserts both the DB flip to idle AND the `session:status` bus emit with `stale-activity-force-idle` in evidence. The `jsc-*` tmux target doesn't exist in the test env so the classifier would return `stopped`/`no tmux session`, but force-idle fires before the classifier decision is acted on — we assert `idle`, not `stopped`.
- `client/src/utils/__tests__/staleBadge.test.ts` (5 tests): threshold constant pin; working + >60s (61/120/999) → true; working + exactly 60s → false (strictly `>`); working + ≤60s (0/30/59) → false; every non-working status × 0/120/999s → always false.

**Verification.** Full suite: server 197/197 unit + 22/22 integration (up from 178 + 22 pre-U). Client 179/179 (up from 174 pre-U). Typecheck green both sides. `pnpm audit --prod` clean (no new deps — chokidar/better-sqlite3/react all present).

**Post-reconstruction checklist.** If this rotation's context is lost, to resume:
1. `git log --oneline 12969cb^..HEAD` should show the 4 Phase U commits.
2. `grep -n "DETERMINISTIC_STRATEGIES\|maybeAutoLearnClaudeSessionId" server/src/routes/hook-event.routes.ts` — one set def + one helper def + one call site in processHook.
3. `grep -n "STALE_ACTIVITY_MS\|stale-activity-force-idle" server/src/services/status-poller.service.ts` — constant, threshold branch, and evidence string.
4. `grep -n "shouldShowStaleBadge\|STALE_BADGE_THRESHOLD_S\|data-testid=\"stale-badge\"" client/src/components/sessions/SessionCard.tsx` — predicate, constant, and JSX chip.
5. `pnpm --filter @commander/server test` → 197/197. `pnpm --filter @commander/server run test:integration` → 22/22. `pnpm --filter @commander/client test` → 179/179.

**Gotchas / invariants.** Keep DETERMINISTIC_STRATEGIES at exactly four — the test pins size=4 and individual has() checks. Anyone adding pm-cwd-rotation or coder-team-rotation to the set must justify it in that test's comments (auto-learn is permanent and a wrong heuristic write cannot be un-done by a later correct one). The force-idle guard's `AND status = 'working'` WHERE clause is load-bearing — concurrent Stop hook + force-idle could otherwise both write and fire duplicate `session:status` events. The stale-badge chip is visibility-only — do NOT add code that mutates state based on it; both applyStaleOverride (30s display override) and the server-side force-idle (90s real flip) are the correct places for any behavior change. The decision-order comment in status-poller.service.ts (`// Ordering: (1) hook yield ... (2) force-idle ... (3) classifier`) is a contract — the force-idle test suite duplicates the decision tree to catch any reorder regression. If anyone moves the force-idle guard before the hook yield, test #7 ("working + 91s stale + JUST got hook 1s ago → yield (hook wins)") will fail and block the commit.
