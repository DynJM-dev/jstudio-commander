...
**PASSED 8/8 by Jose on 2026-04-23.** First run hit three classes of plugin-load failure that required PM-shipped fixes mid-smoke (see §4 D6). Once resolved, end-to-end plugin → sidecar → MCP pipeline validated cleanly with real Claude Code sessions.

**Step-by-step outcomes (dispatch §9 target `Command Center.app`):**

1. **Build + launch** — PASS. `Command Center.app` relaunched cold; SMOKE_DISCIPLINE v1.1 §3.4.1 triad held (shell + sidecar in `ps` as bundle children, window count = 1, window 1280×800 at position 1080,305 within primary 3440×1440 display bounds). (Acc. 2.8.)
2. **Preferences → Plugin tab** — PASS. Monospace install command + Copy button + status "Plugin not detected" baseline visible.
3. **Plugin install** — PASS after D6a. First attempt with `file://` prefix rejected ("Invalid marketplace source format") — Claude Code v2.1.118 wants raw path, no URI scheme. Resolution: PM created `~/.commander/plugin` symlink to bundled path + pasted absolute path `/Users/josemiguelbonilla/.commander/plugin` into Claude Code's marketplace dialog. `/plugin install commander@jstudio` reported success. (Acc. 2.1.)
4. **Trigger first hook** — PASS after D6b + D6c. First prompt to Claude showed 0 hooks registered + 2 load errors per `/doctor`. D6b hypothesis (invalid event names) was wrong — actual root cause per second `/doctor` was `url: invalid_format` on every hook because `${COMMANDER_PORT}` isn't valid URL syntax and Claude Code validates URL format BEFORE env-var expansion. Resolution: `apps/plugin/hooks/forward.sh` bash shim reading `~/.commander/config.json` dynamically, `type: http` → `type: command` across all hooks. After second `/reload-plugins`, one remaining error: "Duplicate hooks file detected" — `plugin.json`'s `hooks: ./hooks/hooks.json` field collides with Claude Code's convention-auto-load. Resolution: dropped the field. Third `/reload-plugins` → 0 errors, 9 hooks registered. Jose typed "what files are in this directory?" → Claude responded → **"Plugin detected"** flipped within 5s. (Acc. 2.7.)
5. **Debug tab recent events** — PASS. Panel showed 8 rows (the 3 pre-existing test/smoke events + 5 new from Jose's real-session chain: `UserPromptSubmit` 16:32:32 → `PreToolUse` 16:32:36 → `PostToolUse` 16:32:37 → `Stop` 16:32:42 → `Notification` 16:33:42, all sharing sid `758e4dfc…`). Timestamps within 60s of each other. Real session_id values. (Acc. 2.2.)
6. **De-dupe via Replay** — PASS. Jose clicked "Replay last event"; UI green toast "Replayed Notification — de-dupe should leave count unchanged" fired. Count moved 8 → 13 but delta was 5 (= fresh prompt Jose ran mid-step firing its own 5-event chain), NOT 6 (which would mean replay also inserted). Replay itself added 0 rows; de-dupe by `session_id + event_uuid` composite PK blocked the duplicate insert as designed per KB-P4.3. (Acc. 2.3.)
7. **Bearer probes** — PASS. `GET /health` → 200, `GET /mcp/tools/list` unauthed → 401, `POST /hooks/session-start -d '{}'` unauthed → **415** initially (Fastify body-parser runs before preHandler auth; no Content-Type header causes 415 before auth even evaluates). Re-ran with `-H "Content-Type: application/json"` → **401** as specified. Auth IS enforced on any parseable request. 415-vs-401 ordering is a middleware detail, not a security gap — logged as §9 Observation 7. (Acc. 2.5.)
8. **MCP external session** — PASS after D6d. First attempt failed with `/mcp` showing `commander · ✘ failed`. Root cause: two compounding issues — (i) PM's initial `~/.claude/settings.json` edit with a top-level `mcpServers` field was rejected by settings.json schema (validator rolled back the edit; correct location is project-root `.mcp.json`); (ii) subsequent `.mcp.json` at `~/Desktop/test-proj-mcp/` used the flat shape seen in plugin-bundled examples, but project-root `.mcp.json` requires the `mcpServers` wrapper (matches discord/external-plugin reference). Also compounded by **unexpected bearer rotation** — sidecar regenerated the bearer token between CODER smoke (`5f77f209…` at ~10:15 UTC) and the MCP test (`69b70ef5…` at 15:59 UTC), so any hardcoded static reference had to be re-synced. After wrapped-shape `.mcp.json` + current bearer, Claude Code connected successfully. Jose prompted `"Using the commander MCP's list_projects tool, show every project…"` → Claude called `list_projects` → 3 real rows returned: `coder-smoke` (10:37 UTC, from T11 dry-run), `tmp` (16:26 UTC, from PM's forward.sh verification test), `test-proj-mcp` (16:42 UTC, auto-created from Jose's second session's SessionStart hook). End-to-end plugin→sidecar→MCP pipeline validated. (Acc. 2.4 + 2.6 verified automated in §3 part 1.)

All acceptance criteria observable in pixels + real MCP tool-call response rendering, not derived properties. N2 close gate met.
...
**D6 (post-filing, PM-shipped 2026-04-23 per Jose standing small-scope authorization) — plugin-load compatibility fixes against Claude Code v2.1.118 runtime.** Four sub-deviations surfaced by Jose's smoke step 3 + step 4 + step 8, fixed mid-rotation rather than routing to CTO for N2.1 hotfix. Commits: `74af4df` + `c429e5a` + `441e007` on commander-repo, plus `.mcp.json` direct edits at `~/Desktop/test-proj-mcp/` (non-tracked, ephemeral test config).

- **D6a — `/plugin install file://...` rejected.** Claude Code v2.1.118's "Add Marketplace" dialog accepts `owner/repo`, `https://...`, `./path` — NOT `file://` URI scheme. Fix: PM created `~/.commander/plugin` symlink to the bundled plugin dir and instructed Jose to paste absolute path (no URI prefix) into the marketplace dialog. KB-P3.2 and dispatch §7 both cited `file://` as the local install method; both need calibration.
- **D6b — `hooks.json` URL validation fails pre-env-var-expansion.** Claude Code v2.1.118 validates `"url"` field of http-type hooks at plugin-load time as strict URL format. `${COMMANDER_PORT}` placeholders fail `invalid_format: url` BEFORE any env-var expansion. KB-P3.1 explicitly recommends HTTP-transport hooks with env-var interpolation — does not work in current Claude Code. Fix: `apps/plugin/hooks/forward.sh` ~50-LOC bash shim reading `~/.commander/config.json` on every invocation (port + bearer, dynamic — absorbs port-scan variance + bearer rotation silently); `hooks.json` rewritten to use `type: command` with `bash "${CLAUDE_PLUGIN_ROOT}/hooks/forward.sh" <event-name>`. Side-finding: all 36 working hook entries across the installed Claude Code marketplace use `type: command` — zero use `type: http`. HTTP transport exists only for MCP servers (`.mcp.json`), not for hooks (`hooks.json`). KB-P3.1's HTTP-hook claim is spec-wrong for current Claude Code; needs calibration pass.
- **D6c — `plugin.json manifest.hooks` field collides with convention auto-load.** Claude Code v2.1.118 auto-loads `.claude-plugin/../hooks/hooks.json` by convention; `manifest.hooks` is reserved for ADDITIONAL hook files at non-standard paths. Pointing it at the standard path causes "Duplicate hooks file detected: already-loaded file" → aborts hook registration. Fix: dropped the `"hooks": "./hooks/hooks.json"` line from `plugin.json`. ARCHITECTURE_SPEC §8.1 + KB-P3.2 both showed this field as required; both need calibration.
- **D6d — project-root `.mcp.json` schema requires `mcpServers` wrapper.** PM initially guided Jose to add `mcpServers` to `~/.claude/settings.json` — settings.json schema explicitly rejects that field (validator rolled back the edit). Correct location is project-root `.mcp.json`. On the `.mcp.json` side, schema is ambiguous across Claude Code's own marketplace examples: 4 of 5 plugin-bundled `.mcp.json` use flat shape (`{ "server-name": {...} }`), 1 of 5 uses wrapped (`{ "mcpServers": { "server-name": {...} } }`). **Project-root `.mcp.json` requires the wrapped shape**; flat shape produces "mcpServers: Does not adhere to MCP server configuration schema" parse error. Distinction: **plugin-bundled** `.mcp.json` is flat-friendly; **project-root** `.mcp.json` requires the wrapper. ARCHITECTURE_SPEC §7.3 needs to specify this explicitly.

None of D6a-d required Rust changes or schema changes. Commit chain affects only `apps/plugin/` + test `.mcp.json`. All four sub-deviations came from Claude Code runtime vs KB-P3.1/P3.2 documentation drift; none are bugs in CODER's N2 implementation relative to the dispatch it was given. Four KB calibration flags stacked for CTO (see §8).
...
**Debt 15 (added post-smoke 2026-04-23) — Bearer token rotated unexpectedly between CODER smoke and MCP external-session test.** D-N1-07 §8.2 spec: "single local bearer token, v1: no expiry, minted on first launch, persisted." Evidence violates this contract — `~/.commander/config.json` showed `updatedAt: 2026-04-23T15:59:03` with a new bearer `69b70ef5-f2a0-4d75-83e9-725802f786f3` replacing the earlier `5f77f209-7d38-47a2-be5d-d986bfb759ca` from CODER's smoke-readiness. Forward.sh absorbed this silently (dynamic config read on every invocation). Anything hardcoded against the token (Jose's `export COMMANDER_TOKEN=...` env var, the original `.mcp.json` PM created, static reference docs) silently broke. **Severity:** MEDIUM — v1 contract violation and users in N3+ setting up external Claude Code sessions can't trust any written-down bearer. **Root cause candidates (not investigated):** (a) sidecar config-persistence path regenerates on certain error conditions; (b) boot-time check mistakes a valid config for "missing/corrupt" and overwrites; (c) specific code path (re-init after client disconnect?) rewrites config.json. **Est. effort:** 2-4 hr — grep sidecar source for `config.json` writes + audit when each write path fires + add a test that asserts bearer stays stable across restarts. **Scheduling:** propose addressing in N2.1 hotfix before N3 — N3's PTY flow will register long-lived agent runs that external Claude Code sessions may reference by bearer across hours/days. Bearer churn mid-run is a reliability hazard.
