# Accessing JStudio Commander from your phone

JStudio Commander runs on your laptop. To reach it from your phone (on the
go, on cellular, on a different Wi-Fi network), Commander spins up a
...
2. **Set a PIN.** Edit `~/.jstudio-commander/config.json`:
   ...
   Pick a numeric PIN at least 4 digits long. **Commander refuses to start the
...
From Commander's UI: open the top command bar (or settings) and hit
...
2. Enter your PIN. Commander remembers it for the tab session
   (sessionStorage — clears when you fully close the browser).
3. **Add to Home Screen** for a near-native feel: Safari → Share → Add to
   Home Screen. The PWA manifest gives Commander a standalone app shell
...
- **Tunnel URL works but PIN fails**: you've hit the brute-force lockout
  ...
  Commander to clear the in-memory counter.
- **Phone shows "Server restarting…" banner**: the laptop's Commander
...
- **Tunnel URL changed after restart**: free trycloudflare URLs rotate on
  ...
  set up by Commander out of the box.
...
- Cloudflared logs URLs at the proxy layer — Commander rejects PIN passed
